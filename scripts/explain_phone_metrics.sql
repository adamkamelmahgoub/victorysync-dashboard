...
filtered_calls AS (
  SELECT
    ...
    status, duration, answered_at, ended_at
  FROM public.calls
...
SELECT p.id, p.number, p.number_digits, COALESCE(a.calls_count,0) as calls_count, COALESCE(a.answered_count,0) as answered_count
FROM org_phones p
