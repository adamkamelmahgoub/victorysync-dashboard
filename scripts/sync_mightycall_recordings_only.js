...
(async () => {
  console.log('Syncing real MightyCall recordings directly into mightycall_recordings...');
  ...
  // Build recordings rows only (bypass calls table insert since it has schema issues)
  const recordingRows = [];
  ...
  for (const c of calls) {
    ...
    if (c.callRecord && (c.callRecord.uri || c.callRecord.fileName)) {
      recordingRows.push({
        ...
        metadata: { caller_phone: c.caller?.phone, business_number: c.businessNumber, callStatus: c.callStatus }
      });
    }
  }
  ...
  if (recordingRows.length > 0) {
    ...
    console.log('Sample recording row:', JSON.stringify(data?.[0] || recordingRows[0], null, 2));
  } else {
    console.log('No recordings with URLs found in MightyCall calls');
  }
  
  console.log('\nSUCCESS: synced real MightyCall recordings into DB');
  process.exit(0);
})();
