...
import salesHubRouter from './routes/salesHub';
...
function collectExtensionCandidates(input: any, depth = 0): string[] {
  ...
  for (const [key, value] of Object.entries(input)) {
    ...
    if (
      ...
      lower === 'user_id' ||
      lower === 'userextension' ||
      lower === 'user_extension' ||
      lower === 'participantextension' ||
      lower === 'participant_extension' ||
      lower === 'answered_by' ||
      lower === 'answeredby' ||
      lower === 'answer_extension' ||
      lower === 'agent_extension' ||
      lower === 'assignedextension' ||
      lower === 'assigned_extension' ||
      lower === 'ownerextension' ||
      lower === 'owner_extension'
    ) {
      if (value && typeof value === 'object') {
        ...
        add((value as any).id);
        add((value as any).extensionNumber);
        add((value as any).internalNumber);
        add((value as any).shortNumber);
      } else {
        add(value);
      }
    }
    ...
    if (
      ...
      lower === 'currentcall' ||
      lower === 'current_call' ||
      lower === 'currentcallinfo' ||
      lower === 'current_call_info'
    ) {
...
function inferAgentLiveStatusFromPayload(payload: any): 'ringing' | 'dialing' | 'answered' | 'in_progress' | 'available' {
  if (!payload || typeof payload !== 'object') return 'available';
  if (!hasFreshStatusCallSignal(payload, 20 * 60 * 1000)) return 'available';
  ...
  const label = String(
    extractLiveStatusLabel(currentCall) ||
    extractLiveStatusLabel(payload) ||
    ''
  ).toLowerCase();

  if (label.includes('ring')) return 'ringing';
  if (label.includes('dial') || label.includes('calling')) return 'dialing';
  if (label.includes('answer')) return 'answered';
  if (label.includes('connect') || label.includes('talk') || label.includes('progress') || label.includes('active') || label.includes('call')) {
    return 'in_progress';
  }
  ...
  if (directionText.includes('out')) return 'dialing';
  return 'ringing';
}

async function resolveOrgAgentUserIdByExtension(orgId: string, extension: string): Promise<string | null> {
  ...
  try {
    const { data: fromOrgUsers } = await supabaseAdmin
      ...
      .limit(1)
      .maybeSingle();
    if (fromOrgUsers?.user_id) return String(fromOrgUsers.user_id);
  } catch {}

  try {
    const { data: fromAgentExtensions } = await supabaseAdmin
      .from('agent_extensions')
      ...
      .limit(1)
      .maybeSingle();
    if (fromAgentExtensions?.user_id) return String(fromAgentExtensions.user_id);
  } catch {}

  return null;
}

async function upsertAgentLiveStatusRow(params: {
  orgId: string;
  extension: string;
  status: string;
...
function extractAgentEmailFromMeta(extMeta: any): string | null {
  const payload = extMeta?.metadata || extMeta || {};
  const candidates = [
    extMeta?.email,
    payload?.email,
    payload?.login,
    ...
    payload?.currentStatus?.email,
    payload?.liveStatus?.email,
  ];
  for (const value of candidates) {
    const text = String(value || '').trim();
...
async function probeLiveStatusesForAgents(agents: RealAgentMember[]): Promise<Map<string, any>> {
  ...
  const uniqueAgents = Array.from(new Map(
    agents
      .filter((agent) => !!agent.org_id && !!agent.mightycall_extension)
      .map((agent) => [`${agent.org_id}:${agent.mightycall_extension}`, agent])
  ).values());
  ...
  await Promise.all(uniqueAgents.map(async (agent) => {
    ...
    try {
      const status = await withDeadline(
        getMightyCallStatusByExtension({ orgId: agent.org_id, extension: agent.mightycall_extension }),
        5000,
        null as any
      );
      if (!status) return;
      const normalized = normalizeAgentLiveEventStatus(status.normalizedStatus || status.rawStatus || 'unknown');
      const presence = mapAgentLiveStatusToPresence(normalized);
      const nowIso = status.lastSyncedAt || new Date().toISOString();
      out.set(key, {
        user_id: agent.user_id || '',
        org_id: agent.org_id,
        email: agent.email || status.email || null,
        role: agent.role || 'agent',
        extension: agent.mightycall_extension,
        display_name: agent.full_name || status.name || inferDisplayNameFromEmail(agent.email || status.email || null) || null,
        on_call: presence.on_call,
        direction: status.direction || null,
        from_number: status.direction === 'inbound' ? (status.counterpartNumber || null) : null,
        to_number: status.direction === 'outbound' ? (status.counterpartNumber || null) : null,
        counterpart: presence.on_call ? (status.counterpartNumber || null) : null,
        status: presence.label,
        started_at: presence.on_call ? (status.statusStartedAt || null) : null,
        answered_at: presence.on_call ? (status.statusStartedAt || null) : null,
        ended_at: null,
        source: 'mightycall_user_status_by_extension',
        raw_status: status.rawStatus || null,
...
const LIVE_AGENT_PRESENCE_SYNC_VERSION = 'db-presence-cache-v42';
...
async function getAgentLiveStatusItemsForOrg(orgId: string): Promise<any[]> {
  const liveCallsWindowStart = new Date(Date.now() - (30 * 60 * 1000)).toISOString();
  const [
  ...
  const memberRows = (membersData || []).filter((row: any) => String(row?.role || '').toLowerCase() === 'agent');
  ...
  const activeMemberRows = memberRows.filter((row: any) => {
    const userId = String(row?.user_id || '');
    const extension = extensionByUserId.get(userId) || String(row?.mightycall_extension || '').trim();
    return !!normalizeExtension(extension);
  });
  if (activeMemberRows.length === 0) return [];

  const userIds = Array.from(new Set(activeMemberRows.map((row: any) => String(row.user_id || '')).filter(Boolean)));
  ...
  const cachedExtensionsPromise = uniqueExtensions.length > 0
    ? supabaseAdmin
        .from('mightycall_extensions')
        .select('extension, display_name, metadata, external_id')
        .in('extension', uniqueExtensions)
    : Promise.resolve({ data: [] as any[] });
  const [
    { data: cachedExtensions },
    { data: recentCallRows, error: recentCallsError },
    resolvedEmails,
  ] = await Promise.all([
    cachedExtensionsPromise,
    supabaseAdmin
      .from('calls')
      .select('id, external_id, from_number, to_number, status, started_at, answered_at, ended_at, duration_seconds, agent_extension, metadata')
      .eq('org_id', orgId)
      .gte('started_at', liveCallsWindowStart)
      .order('started_at', { ascending: false })
      .limit(30),
    withDeadline(resolveUserEmailsById(userIds), 1200, {} as Record<string, string>),
  ]);
  if (recentCallsError) throw recentCallsError;
  const emailByUserId = new Map<string, string | null>();
  for (const [id, email] of Object.entries(resolvedEmails || {})) {
    emailByUserId.set(String(id), email || null);
  }
  ...
  for (const row of cachedExtensions || []) {
    const ext = normalizeExtension((row as any)?.extension);
    if (ext && !extensionMetaByExt.has(ext)) {
  ...
  return activeMemberRows.map((member: any) => {
    ...
    const matchedStoredCall = !matchedLiveCall && !inferredLiveCall && normalizedExt
      ? activeStoredCalls.find((call: any) => {
          const normalizedStoredExt = normalizeExtension(call?.agent_extension || call?.metadata?.agent_extension || call?.metadata?.agent?.extension || null);
          if (normalizedStoredExt && normalizedStoredExt === normalizedExt) return true;
          return payloadMatchesAgent(call?.metadata || call, normalizedExt, agentIdentities);
        }) || null
      : null;
    ...
    const statusPayload = extPayload?.liveStatus || extPayload?.currentStatus || extPayload;
	    ...
	    const authoritativeStatusSaysOnCall = !!authoritativeStatusPayload && (
	      ...
	      (!!authoritativeStatusLabel && isLikelyActiveCallStatus(authoritativeStatusLabel)) ||
	      hasFreshStatusCallSignal(authoritativeStatusPayload)
	    );
	    const authoritativeStatusSaysIdle = !!authoritativeStatusPayload && !authoritativeStatusSaysOnCall && (
...
async function ensureLiveAgentPresenceFresh(orgIds: string[]) {
  ...
  // Kick refresh in the background so read path remains fast and does not time out.
  for (const targetOrgId of orgIdsNeedingRefresh) {
    void refreshLiveAgentPresenceForOrgWithLock(targetOrgId).catch((err: any) => {
      console.warn('[live-agent-presence] async request refresh failed', targetOrgId, fmtErr(err));
    });
  }

  // If there is no cached row at all, wait briefly for first paint.
  if (rows.length === 0) {
    await withDeadline(
  ...
  return {
    rows,
    refreshed: rows.length === 0 ? true : false,
    liveStatusVersion: rows.length === 0
...
async function getMightyCallExtensionInventoryForOrg(orgId: string, liveOnly: boolean) {
    ...
    try {
      ...
      liveFetchError = fmtErr(e) ?? 'unknown_error';
    }
    ...
    try {
      ...
      for (const item of assigned || []) {
        pushRow('agent_extensions', item?.extension, null);
      }
    } catch (e) {
    ...
    try {
      ...
      for (const item of orgUsers || []) {
        pushRow('org_users', item?.mightycall_extension, null);
      }
    } catch (e) {
...
// POST /api/orgs/:orgId/mightycall/extensions/cleanup - remove stale org-scoped extension assignments not present in live MightyCall
app.post('/api/orgs/:orgId/mightycall/extensions/cleanup', async (req, res) => {
  try {
    const actorId = req.header('x-user-id') || null;
    const { orgId } = req.params;
    if (!actorId) return res.status(401).json({ error: 'unauthenticated' });
    ...
    const liveSet = new Set(
      (live || [])
        .map((item: any) => String(item?.extension || '').trim())
        .filter(Boolean)
    );

    if (liveSet.size === 0) {
      return res.status(400).json({ error: 'no_live_extensions_found', detail: 'Cleanup aborted because MightyCall did not return any live extensions.' });
    }
    ...
    const staleAgentExtensions = (agentRows || []).filter((row: any) => {
      const normalized = String(row?.extension || '').trim();
      return normalized && !liveSet.has(normalized);
    });

    if (staleAgentExtensions.length > 0) {
      const staleValues = Array.from(new Set(staleAgentExtensions.map((row: any) => String(row.extension).trim()).filter(Boolean)));
      const { error: deleteAgentError } = await supabaseAdmin
        .from('agent_extensions')
        .delete()
        .eq('org_id', orgId)
        .in('extension', staleValues);
      if (deleteAgentError) throw deleteAgentError;
    }

    const { data: orgUserRows, error: orgUserRowsError } = await supabaseAdmin
      .from('org_users')
      .select('user_id, mightycall_extension')
      .eq('org_id', orgId)
      .not('mightycall_extension', 'is', null);
    if (orgUserRowsError) throw orgUserRowsError;

    const staleOrgUsers = (orgUserRows || []).filter((row: any) => {
      const normalized = String(row?.mightycall_extension || '').trim();
      return normalized && !liveSet.has(normalized);
    });

    if (staleOrgUsers.length > 0) {
      const staleUserIds = Array.from(new Set(staleOrgUsers.map((row: any) => String(row.user_id || '')).filter(Boolean)));
      const { error: clearOrgUsersError } = await supabaseAdmin
        .from('org_users')
        .update({ mightycall_extension: null })
        .eq('org_id', orgId)
        .in('user_id', staleUserIds);
      if (clearOrgUsersError) throw clearOrgUsersError;
    }

    res.json({
      ok: true,
      live_extensions: Array.from(liveSet.values()).sort((a, b) => a.localeCompare(b)),
      removed_agent_extensions: staleAgentExtensions.map((row: any) => ({
        user_id: row.user_id,
        extension: row.extension
      })),
      cleared_org_user_extensions: staleOrgUsers.map((row: any) => ({
        user_id: row.user_id,
        extension: row.mightycall_extension
      })),
      note: 'Global mightycall_extensions cache was left untouched to avoid removing values that may belong to other org integrations.'
    });
  } catch (err: any) {
    console.error('cleanup_org_mightycall_extensions_failed:', fmtErr(err));
    res.status(500).json({ error: 'cleanup_org_mightycall_extensions_failed', detail: fmtErr(err) ?? 'unknown_error' });
  }
});
...
app.post('/api/admin/live-status/simulate', async (req, res) => {
  try {
    const actorId = req.header('x-user-id') || null;
    if (!actorId || !(await isPlatformAdmin(actorId))) {
      return res.status(403).json({ error: 'forbidden' });
    ...
    if (!orgId || !extension || !event) {
      return res.status(400).json({ error: 'orgId, extension, event are required' });
    }
    const nowIso = new Date().toISOString();
    const callId = String(req.body?.callId || `${extension}:${Date.now()}`);
    const fromNumber = String(req.body?.from_number || req.body?.fromNumber || '+15550001111');
    const toNumber = String(req.body?.to_number || req.body?.toNumber || '+15550002222');
    ...
    await upsertAgentLiveStatusRow({
      ...
      direction: String(req.body?.direction || 'inbound').toLowerCase().includes('out') ? 'outbound' : 'inbound',
      fromNumber,
      toNumber,
      startedAt: req.body?.started_at || nowIso,
      answeredAt: event === 'answered' || event === 'in_progress' ? (req.body?.answered_at || nowIso) : null,
      endedAt: event === 'completed' || event === 'missed' || event === 'failed' ? (req.body?.ended_at || nowIso) : null,
      rawEvent: { simulated: true, event, nowIso, body: req.body || {} },
      lastEventAt: nowIso,
    });

    res.json({ ok: true, org_id: orgId, extension, event, call_id: callId });
  } catch (err: any) {
...
app.get('/api/admin/live-status/debug', async (req, res) => {
  try {
    const actorId = req.header('x-user-id') || null;
    if (!actorId || !(await isPlatformAdmin(actorId))) {
      return res.status(403).json({ error: 'forbidden' });
    ...
    const { data: liveRow, error: liveRowError } = await supabaseAdmin
      ...
      .eq('extension', extension)
      .maybeSingle();
    if (liveRowError) throw liveRowError;
    ...
    const token = await withDeadline(getMightyCallAccessToken(overrideCreds), 1500, null as any);
    const apiKeyOverride = overrideCreds?.clientId || undefined;

    const [liveCall, profileStatus] = token
      ? await Promise.all([
          withDeadline(fetchMightyCallLiveCallByExtension(extension, token, apiKeyOverride), 1500, null as any),
          withDeadline(fetchMightyCallProfileStatusByExtension(extension, token, apiKeyOverride), 1500, null as any),
        ])
      : [null, null];
    ...
    res.json({
      ...
      probe: {
        ...
        inferred_from_profile: inferAgentLiveStatusFromPayload(profileStatus),
      },
      now: new Date().toISOString(),
    });
  } catch (err: any) {
    console.error('admin_live_status_debug_failed:', fmtErr(err));
    res.status(500).json({ error: 'admin_live_status_debug_failed', detail: fmtErr(err) ?? 'unknown_error' });
  }
});
