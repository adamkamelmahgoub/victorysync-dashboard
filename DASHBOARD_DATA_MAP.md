...
| Stripe billing | Stripe Checkout, Billing Portal, card payments, webhooks + Supabase billing tables | `/api/billing/stripe/plans`, `/api/billing/stripe/checkout-session`, `/api/billing/stripe/payment-session`, `/api/billing/stripe/portal-session`, `/api/billing/stripe/webhook` | `billing_plans.stripe_price_id`, `org_subscriptions.stripe_customer_id`, `org_subscriptions.stripe_subscription_id`, `invoices.stripe_invoice_id`, `invoices.amount_due`, `invoices.amount_paid`, `billing_records.stripe_*`, `payment_transactions.stripe_payment_intent_id` | Org billing managers/admins; webhook is public but signature verified | Plan cards show missing Stripe price state; invoices show card payment action only when payable; webhook returns signature/config errors | Added hosted subscription Checkout, one-time card payment Checkout for invoices/custom charges, Stripe Billing Portal launch, webhook sync for subscriptions/invoices/payments, and migration `supabase/032_stripe_billing_integration.sql`. |
