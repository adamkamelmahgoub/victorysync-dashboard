...
| Stripe billing | Stripe Checkout, Billing Portal, webhooks + Supabase billing tables | `/api/billing/stripe/plans`, `/api/billing/stripe/checkout-session`, `/api/billing/stripe/portal-session`, `/api/billing/stripe/webhook` | `billing_plans.stripe_price_id`, `org_subscriptions.stripe_customer_id`, `org_subscriptions.stripe_subscription_id`, `invoices.stripe_invoice_id`, `billing_records.stripe_*` | Org billing managers/admins; webhook is public but signature verified | Plan cards show missing Stripe price state; webhook returns signature/config errors | Added hosted Checkout, Stripe Billing Portal launch, webhook sync for subscriptions/invoices, and migration `supabase/032_stripe_billing_integration.sql`. |
