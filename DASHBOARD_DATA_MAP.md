...
| Client portal billing lock | Billing access middleware + manual admin locks | `/api/client/billing/access`, `/api/admin/billing/access-locks`, `/api/admin/orgs/:orgId/billing-lock` | `org_billing_locks`, `org_subscriptions.status`, `invoices.status`, `invoices.due_date`, `invoices.amount_due`, `invoices.amount_paid` | Platform admins can manage locks; clients can only read their own lock state | Locked clients can still access billing, account, and support pages | Added server-side 402 lock enforcement for unpaid/locked client orgs plus Billing page lock/unlock controls. Requires migration `supabase/033_billing_locks_invite_verification.sql`. |
| Invite signup verification | Invite validation + email verification code | `/api/auth/validate-invite`, `/api/auth/signup-with-invite` | `invites`/`org_invites`, `invite_verification_codes`, `organizations` | Public auth flow, but invite must match email + org ID + code | Expired or mismatched codes return clean signup errors | Signup now requires organization ID, invite code assigned to the same email, and a six-digit email verification code before account creation. |
| Theme + MFA settings | User profile + Supabase Auth MFA | `/api/user/profile`, Supabase `auth.mfa` client API | `profiles.metadata.theme`, Supabase Auth MFA factors | Authenticated users | Shows error if Supabase MFA is not enabled/configured | Added real light/dark mode persistence and account settings controls for TOTP 2FA enrollment/removal. |
